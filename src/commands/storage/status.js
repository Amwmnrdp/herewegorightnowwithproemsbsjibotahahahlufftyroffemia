...
async function execute(interaction, langCode) {
    ...
    const emojiStatus = (emojiAllowed ? '🟢 ' : '🔴 ') + (emojiAllowed ? await t('Granted', langCode) : await t('Denied', langCode));
    
    // 2. Sticker Permission
    const stickerAllowed = perms ? perms.sticker_permission_enabled : true;
    const stickerStatus = (stickerAllowed ? '🟢 ' : '🔴 ') + (stickerAllowed ? await t('Granted', langCode) : await t('Denied', langCode));
    
    // 3. Delete Permission
    const deleteAllowed = perms ? perms.delete_permission_enabled : true;
    const deleteStatus = (deleteAllowed ? '🟢 ' : '🔴 ') + (deleteAllowed ? await t('Granted', langCode) : await t('Denied', langCode));
