...
const pool = new Pool({
    connectionString: connectionString ? connectionString.replace('sslmode=require', 'sslmode=verify-full') : connectionString,
    ssl: {
...
async function addServer(serverId, serverName) {
    await pool.query(
        'INSERT INTO servers (server_id, server_name, emoji_permission_enabled, sticker_permission_enabled, delete_permission_enabled) VALUES ($1, $2, true, true, false) ON CONFLICT (server_id) DO UPDATE SET server_name = $2',
        [serverId, serverName]
    );
}
...
async function setEmojiPermission(serverId, enabled) {
    await pool.query(
        'UPDATE servers SET emoji_permission_enabled = $1 WHERE server_id = $2',
        [enabled, serverId]
    );
}

async function setStickerPermission(serverId, enabled) {
    await pool.query(
        'UPDATE servers SET sticker_permission_enabled = $1 WHERE server_id = $2',
        [enabled, serverId]
    );
}
...
async function getServerPermissions(serverId) {
    ...
    if (result.rows.length === 0) {
        return {
            emoji_permission_enabled: true,
            sticker_permission_enabled: true,
